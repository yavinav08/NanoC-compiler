...
%%
program: _PROGRAM progname _VAR declist _BEGIN stmtlist _END_DOT { FILE*file = fopen("symtab.txt","w"); print_hash_table(file); fclose(file);} ; | 

progname: _ID;

declist: dec| declist _SEMICOLON dec;

dec: idlist _COLON type ;

type: _INTEGER { if(!set_type(0)) 
...
stmtlist: stmt| stmtlist _SEMICOLON stmt;
...
assign: _ID _ASSIGN exp { 
			...
			yyerror("declaration is missing ");// error not declared symbol
	} ;
...
exp: exp _ADD term {
			...
			yyerror("mismatch type on addition");
			$$.type = 1;
			if($1.type == 0)
   ...
   | exp _SUB term {
			...
			yyerror("mismatch type on subtraction");
			$$.type = 1;
			if($1.type == 0)
				$$.real = $1.integer - $3.real;
			else
...
term: term _MUL factor {
			...
			yyerror("mismatch type on multiplication");
			$$.type = 1;
			if($1.type == 0)
				$$.real = $1.integer * $3.real;
			else
    ...
    | term _DIV factor { 
			...
			yyerror("mismatch type on division");
			$$.type = 1;
			if($1.type == 0)
				$$.real = $1.integer / $3.real;
			else
...
factor: _OPEN_BRACE exp _CLOSE_BRACE { $$ = $2; }
      ...
      | _ID { 
			...
			if(is_initialized($1)) {
				...
				yyerror("variable not been initialised");
...
read: _READ _OPEN_BRACE idlist _CLOSE_BRACE {
	if(!set_init())
		yyerror("declaration is missing ");} ;
...
indexexp: _ID _ASSIGN exp _TO exp {
		if($3.type != $5.type) {
			yyerror("mismatch type on assignment");
		}
      		if(is_declared($1)) {
			if($3.type == get_type($1)) {
				if($3.type == 0) 
				...
				yyerror("mismatch type on assignment");// expression assignment mismatch
		}
		else
			yyerror("missing declaration");// error not declared symbol
		
	};

body: _BEGIN stmtlist _END| stmt;
